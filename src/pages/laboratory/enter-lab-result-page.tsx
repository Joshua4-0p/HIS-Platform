...
function authHeader(): HeadersInit {
  return {
    ...
    Authorization: `Bearer ${localStorage.getItem("his_id_token") ?? ""}`,
  }
}

// ── Test reference catalogue (frontend live preview only) ───────────────
...
const FALLBACK_REF: TestRef = {
  ...
  normalMin: 0, normalMax: 9999, criticalLow: 0, criticalHigh: 99999,
}
...
export function EnterLabResultPage() {
  ...
  useEffect(() => {
    if (!requestId) return
    setLoadingReq(true)
    fetch(`${API_BASE}/laboratory/requests/${requestId}`, { headers: authHeader() })
      .then(r => r.json())
      .then((data: { request: LabRequestDetail }) => setRequest(data.request))
      .catch(() => setReqError("Failed to load lab request details."))
    ...
    try {
      const res = await fetch(`${API_BASE}/laboratory/results`, {
        ...
        body: JSON.stringify({
          ...
          dateTimeTested: new Date(testDate).toISOString(),
        }),
      })
      if (!res.ok) {
        const err = await res.json().catch(() => ({ error: "Unknown error" }))
        toast.error("Submission Failed", { description: (err as { error: string }).error })
        return
      }
  ...
  if (reqError || !request) {
    return (
      <div className="space-y-4">
        ...
        <div className="flex min-h-64 items-center justify-center rounded-lg border border-dashed border-border bg-card">
          <p className="text-sm text-muted-foreground">{reqError ?? "Lab request not found."}</p>
        </div>
      </div>
    )
  }
  ...
  if (request.status === "Completed") {
    return (
      <div className="space-y-4">
        ...
        <div className="flex min-h-64 flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-border bg-card">
          <CheckCircle size={32} className="text-[#10B981]" />
          <p className="text-sm text-muted-foreground">This lab request already has a result.</p>
        </div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="mx-auto max-w-140 rounded-lg border border-border bg-card shadow-sm">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="text-sm font-medium text-foreground">
                  {request.patientName}{" "}
                  <span className="font-normal text-muted-foreground">· {request.patientNumber}</span>
                </p>
              </div>
              <div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                <input
                  ...
                  type="text"
                  readOnly
                  value="Auto-populated from your session"
                  className={cn(
                    "w-full cursor-not-allowed rounded-md border border-input bg-muted py-2 pl-3 pr-10 text-sm text-muted-foreground focus:outline-none",
                  )}
                />
        ...
        <div className="flex items-center justify-end gap-3 rounded-b-lg border-t border-border bg-muted/40 px-6 py-4">
          ...
          >
            <CheckCircle size={15} /> {submitting ? "Submitting..." : "Submit Result"}
          </button>
        </div>
      </div>
    </div>
