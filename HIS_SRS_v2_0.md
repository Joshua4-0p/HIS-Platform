...
| **REQ-F-010** | **High** | A role SHALL consist of a named bundle of granular permissions. The system SHALL provide the following 33 atomic permissions: **Patient** - patient:create, patient:read, patient:update, patient:delete (soft-deactivate only), patient:amend; **Appointments** - appointment:create, appointment:read, appointment:update, appointment:cancel; **Clinical Encounters** - encounter:create, encounter:read; **Diagnoses** - diagnosis:create, diagnosis:read; **Vital Signs** - vitals:create, vitals:read; **Prescriptions** - prescription:create, prescription:read; **Lab Results** - lab_result:create, lab_result:read, lab_result:update (versioned correction only, no overwrite per REQ-F-025); **Bulk Upload** - bulk_upload:create, bulk_upload:read; **Transfers** - transfer:request, transfer:approve; **Analytics** - analytics:view; **Staff Management** - staff:create, staff:read, staff:update, staff:deactivate; **Role Management** - role:create, role:update, role:delete, role:assign. Clinical resource permissions (encounter, diagnosis, vitals, prescription, lab_result) have no delete variant per REQ-F-025. The audit log has no delete permission per REQ-F-070. | Granular permissions enable precise, least-privilege access control across all resource types. |
